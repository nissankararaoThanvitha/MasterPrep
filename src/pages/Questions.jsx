  ...
  const toggleAnswer = (id) => {
    ...
    if (!viewed.includes(id)) {
      ...
      localStorage.setItem(
        `viewedQuestions_${email}`,
        JSON.stringify(updated)
      );
    }
  };

  const handleBookmark = (id, title, category, answer, subject) => {
    if (!email) return;
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <select
              ...
              className="px-3 py-1.5 rounded-full border border-gray-300 text-[13px]"
            >
      ...
      {totalPages > 1 && (
        <div className="flex justify-between items-center mt-5">
          <button
            ...
            className="px-4 py-2 rounded-md border bg-white text-sm disabled:opacity-50"
          >
            ← Previous
          </button>
